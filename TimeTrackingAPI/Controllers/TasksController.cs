        ...
        ILogger<TasksController> logger) : ControllerBase
{
    ...
    [HttpGet]
    [ProducesResponseType(typeof(ApiResponse<List<WorkTask>>), 200)]
    [ProducesResponseType(typeof(ApiResponse<List<WorkTask>>), 500)]
    public async Task<ActionResult<ApiResponse<List<WorkTask>>>> GetTasks()
    {
        logger.LogInformation("Получен запрос на получение всех задач");
    ...
    [ProducesResponseType(typeof(ApiResponse<List<WorkTask>>), 200)]
    [ProducesResponseType(typeof(ApiResponse<List<WorkTask>>), 500)]
    public async Task<ActionResult<ApiResponse<List<WorkTask>>>> GetActiveTasks()
    {
        logger.LogInformation("Получен запрос на получение только активных задач");
    ...
    public async Task<ActionResult<ApiResponse<WorkTask>>> GetTask(int id)
    {
        logger.LogInformation("Получен запрос на получение задачи с ID: {Id}", id);
    ...
    /// <response code="400">Некорректные данные запроса или задача с таким названием уже существует в проекте</response>
    /// <response code="500">Внутренняя ошибка сервера</response>
    [HttpPost]
    [ProducesResponseType(typeof(ApiResponse<WorkTask>), 201)]
    [ProducesResponseType(typeof(ProblemDetails), 400)]
    [ProducesResponseType(typeof(ApiResponse<WorkTask>), 500)]
    public async Task<ActionResult<ApiResponse<WorkTask>>> CreateTask(TaskCreateDto taskDto)
    {
        logger.LogInformation("Получен запрос на создание задачи: {Name} для проекта {ProjectId}", 
    ...
    [ProducesResponseType(typeof(ApiResponse<WorkTask>), 404)]
    [ProducesResponseType(typeof(ApiResponse<WorkTask>), 500)]
    public async Task<ActionResult<ApiResponse<WorkTask>>> UpdateTask(int id, TaskUpdateDto taskDto)
    {
        logger.LogInformation("Получен запрос на обновление задачи с ID: {Id}", id);
        ...
        var response = await taskService.UpdateTaskAsync(id, task);
    ...
    /// <response code="400">Задачу нельзя удалить из-за наличия связанных записей времени</response>
    /// <response code="404">Задача не найдена</response>
    /// <response code="500">Внутренняя ошибка сервера</response>
    [HttpDelete("{id:int}")]
    ...
    {
        logger.LogInformation("Получен запрос на удаление задачи с ID: {Id}", id);
