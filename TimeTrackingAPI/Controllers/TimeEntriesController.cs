        ...
        ILogger<TimeEntriesController> logger) : ControllerBase
{
    ...
    [HttpGet]
    [ProducesResponseType(typeof(ApiResponse<List<TimeEntry>>), 200)]
    [ProducesResponseType(typeof(ApiResponse<List<TimeEntry>>), 500)]
    public async Task<ActionResult<ApiResponse<List<TimeEntry>>>> GetTimeEntries()
    {
        logger.LogInformation("Получен запрос на получение всех проводок времени");
            
        var response = await timeEntryService.GetAllTimeEntriesAsync();
    ...
    [ProducesResponseType(typeof(ApiResponse<List<TimeEntry>>), 200)]
    [ProducesResponseType(typeof(ApiResponse<List<TimeEntry>>), 500)]
    public async Task<ActionResult<ApiResponse<List<TimeEntry>>>> GetTimeEntriesByDate(DateTime date)
    {
        logger.LogInformation("Получен запрос на получение проводок за дату: {Date}", 
                date.ToString("yyyy-MM-dd"));
            
        var response = await timeEntryService.GetTimeEntriesByDateAsync(date);
    ...
    [ProducesResponseType(typeof(ApiResponse<List<TimeEntry>>), 200)]
    [ProducesResponseType(typeof(ApiResponse<List<TimeEntry>>), 400)]
    [ProducesResponseType(typeof(ApiResponse<List<TimeEntry>>), 500)]
    public async Task<ActionResult<ApiResponse<List<TimeEntry>>>> GetTimeEntriesByMonth(int year, int month)
    {
        logger.LogInformation("Получен запрос на получение проводок за месяц: {Year}-{Month:00}", year, month);
        ...
        var response = await timeEntryService.GetTimeEntriesByMonthAsync(year, month);
    ...
    public async Task<ActionResult<ApiResponse<TimeEntry>>> CreateTimeEntry(TimeEntryCreateDto timeEntryDto)
    {
        logger.LogInformation("Получен запрос на создание проводки времени на дату {Date} для задачи {TaskId}", 
                timeEntryDto.Date.ToString("yyyy-MM-dd"), timeEntryDto.TaskId);

        // Проверка активности задачи
        var task = await context.WorkTasks
                .AsNoTracking()
                .FirstOrDefaultAsync(t => t.Id == timeEntryDto.TaskId);
        ...
        var validationResult = await ValidateDailyHours(timeEntryDto.Date, timeEntryDto.Hours);
        ...
        var response = await timeEntryService.CreateTimeEntryAsync(timeEntry);
    ...
    [ProducesResponseType(typeof(ApiResponse<List<DailyHoursSummaryDto>>), 200)]
    [ProducesResponseType(typeof(ApiResponse<List<DailyHoursSummaryDto>>), 500)]
    public async Task<ActionResult<ApiResponse<List<DailyHoursSummaryDto>>>> GetDailySummary()
    {
        logger.LogInformation("Получен запрос на получение дневной сводки часов");
            
        var response = await timeEntryService.GetDailySummaryAsync();
    ...
    private async Task<(bool IsValid, string ErrorMessage)> ValidateDailyHours(DateTime date, decimal hours, int? excludeId = null)
    {
        try
        {
            logger.LogInformation("Валидация дневных часов для даты {Date}, добавляемых часов {Hours}", 
            ...
            var existingHours = await query.SumAsync(te => te.Hours);
            var totalHours = existingHours + hours;

            logger.LogInformation("Существующие часы: {ExistingHours}, добавляемые: {Hours}, итого: {TotalHours}", 
                    existingHours, hours, totalHours);

            if (totalHours > 24)
            {
                return (false, $"Сумма часов за день {date:yyyy-MM-dd} не может превышать 24 часа. " +
        ...
        {
            logger.LogError(ex, "Ошибка при валидации дневных часов");
