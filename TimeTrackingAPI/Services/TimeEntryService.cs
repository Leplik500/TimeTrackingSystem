        ...
        ILogger<TimeEntryService> logger) : ITimeEntryService
{
    ...
    public async Task<ApiResponse<List<TimeEntry>>> GetAllTimeEntriesAsync()
    {
        try
        {
            logger.LogInformation("Запрос на получение всех проводок времени");
            ...
            logger.LogInformation("Получено {Count} проводок времени", timeEntries.Count);
        ...
        {
            logger.LogError(ex, "Ошибка при получении списка проводок времени");
    ...
    public async Task<ApiResponse<List<TimeEntry>>> GetTimeEntriesByDateAsync(DateTime date)
    {
        try
        {
            logger.LogInformation("Запрос проводок за дату: {Date}", date.ToString("yyyy-MM-dd"));
            ...
            logger.LogInformation("Получено {Count} проводок за дату {Date}", 
                    timeEntries.Count, date.ToString("yyyy-MM-dd"));
        ...
        {
            logger.LogError(ex, "Ошибка при получении проводок за дату {Date}", date);
    ...
    public async Task<ApiResponse<List<TimeEntry>>> GetTimeEntriesByMonthAsync(int year, int month)
    {
        try
        {
            logger.LogInformation("Запрос проводок за месяц: {Year}-{Month:00}", year, month);
            ...
            logger.LogInformation("Получено {Count} проводок за {Year}-{Month:00}", 
        ...
        {
            logger.LogError(ex, "Ошибка при получении проводок за месяц {Year}-{Month}", year, month);
    ...
    public async Task<ApiResponse<TimeEntry>> CreateTimeEntryAsync(TimeEntry timeEntry)
    {
        try
        {
            logger.LogInformation("Создание новой проводки времени на дату {Date} для задачи {TaskId}", 
                    timeEntry.Date.ToString("yyyy-MM-dd"), timeEntry.TaskId);
            ...
            logger.LogInformation("Проводка времени успешно создана с ID: {Id}", timeEntry.Id);
        ...
        {
            logger.LogError(ex, "Ошибка при создании проводки времени");
    ...
    public async Task<ApiResponse<List<DailyHoursSummaryDto>>> GetDailySummaryAsync()
    {
        try
        {
            logger.LogInformation("Запрос суммарной информации о часах по дням");
            ...
            var result = dailySummary.Select(ds => new DailyHoursSummaryDto
            {
                Date = ds.Date,
                TotalHours = ds.TotalHours,
                Status = GetDailyStatus(ds.TotalHours)
            }).ToList();
                
            logger.LogInformation("Получена сводка по {Count} дням", result.Count);
                
            return ApiResponse<List<DailyHoursSummaryDto>>.Success(
                    result,
                    $"Получена сводка по {result.Count} дням");
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Ошибка при получении дневной сводки");
    ...
    /// </summary>
    /// <param name="totalHours">Общее количество часов</param>
    /// <returns>Статус дня</returns>
