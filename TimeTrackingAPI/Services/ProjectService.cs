        ...
        ILogger<ProjectService> logger) : IProjectService
{
            ...
            GetAllProjectsAsync()
    {
        try
        {
            ...
            logger.LogInformation("Получено {Count} проектов",
                    projects.Count);
    ...
    public async Task<ApiResponse<Project>> GetProjectByIdAsync(
            int id)
    {
        try
        {
            logger.LogInformation(
                    "Запрос на получение проекта с ID: {Id}",
                    id);
            ...
            if (project == null)
            {
                logger.LogWarning("Проект с ID {Id} не найден",
                        id);
    ...
    public async Task<ApiResponse<Project>> CreateProjectAsync(
            Project project)
    {
        try
        {
            logger.LogInformation(
                    "Создание нового проекта: {Name}",
                    project.Name);
            ...
            {
                logger.LogWarning(
                        "Проект с кодом {Code} уже существует",
                        project.Code);
    ...
    public async Task<ApiResponse<Project>> UpdateProjectAsync(
            int id,
            Project project)
    {
        try
        {
            ...
            {
                logger.LogWarning(
                        "Проект с ID {Id} не найден для обновления",
                        id);
            ...
            logger.LogInformation(
                    "Проект с ID {Id} успешно обновлен", id);
    ...
    public async Task<ApiResponse<bool>> DeleteProjectAsync(
            int id)
    {
        try
        {
            logger.LogInformation("Удаление проекта с ID: {Id}",
                    id);
            ...
            {
                logger.LogWarning(
                        "Проект с ID {Id} не найден для удаления",
                        id);
        ...
        {
            logger.LogError(ex,
                    "Ошибка при удалении проекта с ID {Id}", id);
